...
import us.jcedeno.deltauhc.bukkit.scenarios.annotations.Scenario;
...
public class Cutclean extends ListenerScenario {
    ...
    private static int fortuneMultiplier(final ItemStack itemstack) {
        if (itemstack.getType() == Material.AIR || !itemstack.containsEnchantment(Enchantment.FORTUNE))
            return 1;
        final int fortuneLevel = itemstack.getEnchantmentLevel(Enchantment.FORTUNE);
